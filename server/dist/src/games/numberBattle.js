...
exports.createNBState = createNBState;
exports.applyNBChoice = applyNBChoice;
exports.nextNBRound = nextNBRound;
const TOTAL_ROUNDS = 7;
const HAND_SIZE = 5;
function genNumbers(exclude = []) {
    const nums = [];
    while (nums.length < HAND_SIZE) {
        const n = Math.floor(Math.random() * 100) + 1;
        if (!exclude.includes(n) && !nums.includes(n))
            nums.push(n);
    }
    return nums;
}
function createNBState(playerIds) {
...
function applyNBChoice(state, playerId, number) {
    if (state.revealed)
        return null;
    if (state.gameWinner)
        return null;
