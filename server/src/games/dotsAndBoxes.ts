import { DotsAndBoxesState } from '../../../shared/types';

const GRID = 4; // 4x4 boxes = 5x5 dots

export function createDABState(playerIds: [string, string]): DotsAndBoxesState {
  return {
    gridSize: GRID,
...
export function applyDABEdge(
  state: DotsAndBoxesState,
  playerId: string,
  type: 'h' | 'v',
  row: number,
  col: number,
): DotsAndBoxesState | null {
  if (state.isDone || state.winner) return null;
  ...
  if (type === 'h') {
    ...
    if (hEdges[row][col] !== null) return null;
    hEdges[row][col] = playerId;
  } else {
    if (row < 0 || row >= GRID || col < 0 || col > GRID) return null;
    if (vEdges[row][col] !== null) return null;
