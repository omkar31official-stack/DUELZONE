import { TargetRushState } from '../../../shared/types';
...
function makeHands(playerIds: string[]): Record<string, number[]> {
  return Object.fromEntries(playerIds.map(id => [id, genNumbers()]));
}

function makeNullChoices(playerIds: string[]): Record<string, null> {
...
export function createTargetRushState(playerIds: string[]): TargetRushState {
  return {
    round: 1,
...
export function applyTargetRushChoice(
  state: TargetRushState,
  playerId: string,
  number: number,
): TargetRushState | null {
  if (state.revealed || state.gameWinner) return null;
  if (!(playerId in state.choices)) return null;
  ...
  for (const id of players) {
    const diff = Math.abs((choices[id] as number) - state.target);
    if (diff < bestDiff) {
      bestDiff = diff;
      winners = [id];
...
export function nextTargetRushRound(state: TargetRushState): TargetRushState {
  const players = Object.keys(state.scores);
  return {
    ...state,
    round: state.round + 1,
