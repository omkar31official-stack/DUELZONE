import { FIND_MATCH_SYMBOLS, FIND_MATCH_SYMBOL_COUNTS, FindMatchSymbolId } from '../../../shared/constants';
import { FindMatchRoundState, FindMatchSymbol } from '../../../shared/types';
...
export function generateFindMatchRound(
...
): Omit<FindMatchRoundState, 'startedAt' | 'phase' | 'winner'> {
  const symbolPool = [...FIND_MATCH_SYMBOLS] as string[];
  const count = FIND_MATCH_SYMBOL_COUNTS[difficulty] ?? 7;
  ...
  function makeSymbols(ids: string[]): FindMatchSymbol[] {
...
export function validateFindMatchGeneration(iterations = 10000): { passed: boolean; errors: string[] } {
  const errors: string[] = [];
  for (let i = 0; i < iterations; i++) {
    ...
    if (!p1Ids.includes(common)) errors.push(`Round ${i}: common not in p1`);
    if (!p2Ids.includes(common)) errors.push(`Round ${i}: common not in p2`);

    // 2) No duplicates within each board
    if (new Set(p1Ids).size !== p1Ids.length) errors.push(`Round ${i}: p1 has duplicates`);
    if (new Set(p2Ids).size !== p2Ids.length) errors.push(`Round ${i}: p2 has duplicates`);

    // 3) Exactly ONE common symbol between boards
    const overlap = p1Ids.filter((id) => p2Ids.includes(id));
    if (overlap.length !== 1) errors.push(`Round ${i}: expected 1 common, got ${overlap.length}: ${overlap}`);
    if (overlap[0] !== common) errors.push(`Round ${i}: overlap is ${overlap[0]} but common is ${common}`);

    if (errors.length > 20) break; // stop early if many failures
  }
