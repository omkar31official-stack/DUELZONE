...
export interface ArcheryState { phase: 'countdown' | 'playing' | 'result'; scores: Record<string, number>; endTime: number | null; winner: string | null; }

export function createArcheryState(players: string[]): ArcheryState {
  const scores: Record<string, number> = {};
  players.forEach((p) => { scores[p] = 0; });
  return { phase: 'countdown', scores, endTime: null, winner: null };
}

export function handleArcheryAction(state: ArcheryState, player: string, action: any): ArcheryState {
  if (action.type === 'START') return { ...state, phase: 'playing', endTime: Date.now() + QUICK_TAP_DURATION_MS };
  if (action.type === 'SHOOT' && state.phase === 'playing') {
