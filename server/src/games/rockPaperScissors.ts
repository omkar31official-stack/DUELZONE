import { RPSState, RPSChoice } from '../../../shared/types';
import { RPS_WIN_SCORE } from '../../../shared/constants';

export function createRPSState(playerIds: [string, string]): RPSState {
  return {
    round: 1,
    totalRounds: RPS_WIN_SCORE * 2 - 1,
...
export function applyRPSChoice(
  ...
  const gameWinner = players.find(p => scores[p] >= RPS_WIN_SCORE) ?? null;
...
export function nextRPSRound(state: RPSState): RPSState {
