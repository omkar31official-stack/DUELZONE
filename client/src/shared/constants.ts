import type { GameDefinitionMeta } from './types';

export const ROOM_CODE_LENGTH = 6;
export const ROOM_EXPIRE_EMPTY_MS = 10 * 60 * 1000;      // 10 min
export const ROOM_EXPIRE_ONE_PLAYER_MS = 60 * 1000;      // 60 sec
export const MAX_CHAT_LENGTH = 120;
export const MAX_PLAYER_NAME_LENGTH = 16;
export const MAX_ROOM_PLAYERS = 5;
export const QUICK_TAP_DURATION_MS = 20_000;
export const REACTION_DUEL_ROUNDS = 5;
export const RPS_WIN_SCORE = 5;
export const FIND_MATCH_DEFAULT_ROUNDS = 10;
export const CHAT_EMOTES = ['🔥', '😂', '👏', '🎯', '⚡', '💩', 'GG', '👑'];
export const REACTION_EMOTES = ['😂', '🔥', '😱', '😎', '😭', '😡', '👏', '❤️', '💀', '🤣', '👀', 'WOW', 'GG'];

export const ACCENT_COLORS = [
  '#FF6B6B', '#FF9F43', '#FFEAA7', '#55EFC4',
  '#74B9FF', '#A29BFE', '#FD79A8', '#00CEC9',
  '#6C5CE7', '#E17055',
];

export const AVATAR_SEEDS = [
  'dragon', 'tiger', 'wolf', 'eagle', 'shark',
  'phoenix', 'cobra', 'lion', 'fox', 'panther',
  'bear', 'falcon', 'viper', 'hawk', 'jaguar',
];

export const ALL_GAMES: GameDefinitionMeta[] = [
  {
    id: 'ping-pong',
    name: 'Ping Pong Duel',
    description: 'Fast-paced 2-player arcade table tennis. Move your paddle and smash the ball!',
    category: 'SPORTS',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Medium',
    icon: '🏓',
    featured: true,
    popular: true,
    isNew: true,
  },
  {
    id: 'air-hockey',
    name: 'Air Hockey Blitz',
    description: 'Flick your mallet to score puck goals against your opponent!',
    category: 'SPORTS',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-3 min',
    difficulty: 'Medium',
    icon: '🏒',
    featured: true,
    popular: true,
    isNew: true,
  },
  {
    id: 'spinner-battle',
    name: 'Sumo Spinner Battle',
    description: 'Charge and ram your spinner to knock rivals out of the arena ring!',
    category: 'ACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-3 min',
    difficulty: 'Medium',
    icon: '🌀',
    featured: true,
    isNew: true,
  },
  {
    id: 'snake-duel',
    name: 'Snake Duel Arena',
    description: 'Eat energy apples, grow your tail, and trap your rival in the grid!',
    category: 'ACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Hard',
    icon: '🐍',
    popular: true,
    isNew: true,
  },
  {
    id: 'mini-golf',
    name: 'Mini Golf Master',
    description: 'Aim your shot angle and power to sink golf balls around obstacles!',
    category: 'SPORTS',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '⛳',
    isNew: true,
  },
  {
    id: 'tug-of-war',
    name: 'Tug of War Smash',
    description: 'Tap rapidly to pull the rope past your opponent line!',
    category: 'REFLEX',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1 min',
    difficulty: 'Easy',
    icon: '🪢',
    popular: true,
    isNew: true,
  },
  {
    id: 'whack-mole',
    name: 'Whack-a-Mole Rush',
    description: 'Smash moles as soon as they pop up to gain points!',
    category: 'REFLEX',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-2 min',
    difficulty: 'Easy',
    icon: '🐹',
    isNew: true,
  },
  {
    id: 'hand-slap',
    name: 'Hand Slap Duel',
    description: 'Attacker slaps, defender dodges! Fake out your opponent.',
    category: 'REFLEX',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-2 min',
    difficulty: 'Hard',
    icon: '✋',
    isNew: true,
  },
  {
    id: 'penalty-kicks',
    name: 'Penalty Shootout',
    description: 'Take turns shooting and goaltending! Aim carefully.',
    category: 'SPORTS',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Medium',
    icon: '⚽',
    featured: true,
    isNew: true,
  },
  {
    id: 'ultimate-ttt',
    name: 'Ultimate Tic Tac Toe',
    description: 'Tactical 9-board nested Tic Tac Toe strategy duel!',
    category: 'STRATEGY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-8 min',
    difficulty: 'Hard',
    icon: '❌',
    isNew: true,
  },
  {
    id: 'find-match',
    name: 'Find Match',
    description: 'Spot the identical symbol before your opponent!',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Medium',
    icon: '🔍',
    popular: true,
  },
  {
    id: 'tic-tac-toe',
    name: 'Tic Tac Toe',
    description: 'Classic 3-in-a-row strategy. First to three wins!',
    category: 'STRATEGY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-2 min',
    difficulty: 'Easy',
    icon: '❌',
  },
  {
    id: 'connect-four',
    name: 'Connect Four',
    description: 'Drop pieces and connect four in a row to win.',
    category: 'STRATEGY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🔵',
    popular: true,
  },
  {
    id: 'rock-paper-scissors',
    name: 'Rock Paper Scissors',
    description: 'Best of 9 rounds. Outsmart your opponent!',
    category: 'QUICK',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-3 min',
    difficulty: 'Easy',
    icon: '✊',
  },
  {
    id: 'reaction-duel',
    name: 'Reaction Duel',
    description: 'Fastest finger wins. Press GO the instant you see it!',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1-2 min',
    difficulty: 'Easy',
    icon: '⚡',
    featured: true,
  },
  {
    id: 'quick-tap',
    name: 'Quick Tap',
    description: 'Tap as many times as possible in 10 seconds.',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1 min',
    difficulty: 'Easy',
    icon: '👆',
  },
  {
    id: 'memory-duel',
    name: 'Memory Duel',
    description: 'Flip cards and find matching pairs before your opponent.',
    category: 'BOARD',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🃏',
  },
  {
    id: 'number-battle',
    name: 'Number Battle',
    description: 'Choose the number closest to the target each round.',
    category: 'QUICK',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-3 min',
    difficulty: 'Easy',
    icon: '🔢',
  },
  {
    id: 'color-clash',
    name: 'Color Clash',
    description: 'Name the ink color, not the word! Stroop effect duel.',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-3 min',
    difficulty: 'Hard',
    icon: '🎨',
  },
  {
    id: 'dots-and-boxes',
    name: 'Dots & Boxes',
    description: 'Draw lines to complete boxes. Most boxes wins!',
    category: 'STRATEGY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '4-7 min',
    difficulty: 'Medium',
    icon: '⬜',
  },
  {
    id: 'tap-royale',
    name: 'Tap Royale',
    description: 'A 10-second button-mashing arena for up to five players.',
    category: 'PARTY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '1 min',
    difficulty: 'Easy',
    icon: '👑',
  },
  {
    id: 'target-rush',
    name: 'Target Rush',
    description: 'Everyone picks the number closest to the target. Best score wins.',
    category: 'PARTY',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-3 min',
    difficulty: 'Medium',
    icon: '🎯',
  },
  {
    id: 'word-scramble',
    name: 'Word Scramble',
    description: 'Unscramble the letters to form the correct word first!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🔤',
  },
  {
    id: 'trivia-blitz',
    name: 'Trivia Blitz',
    description: 'Answer rapid-fire trivia questions faster than your rivals!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🧠',
    featured: true,
  },
  {
    id: 'speed-math',
    name: 'Speed Math',
    description: 'Solve mental math equations faster than everyone else!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Medium',
    icon: '➕',
  },
  {
    id: 'pattern-master',
    name: 'Pattern Master',
    description: 'Memorize and repeat the glowing sequence pattern!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Hard',
    icon: '🧩',
  },
  {
    id: 'pic-combo',
    name: 'Pic Combo',
    description: 'Combine the 2 picture symbols to guess the secret word!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Easy',
    icon: '🖼️',
  },
  {
    id: 'archery',
    name: 'Archery Duel',
    description: 'Pull back, aim, and hit the bullseye!',
    category: 'ARCADE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🏹',
    popular: true,
  },
  {
    id: 'bowling',
    name: 'Pro Bowling',
    description: 'Flick to roll your ball. Get a strike!',
    category: 'ARCADE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '4-6 min',
    difficulty: 'Medium',
    icon: '🎳',
  },
  {
    id: 'hammer',
    name: 'Hammer Smash',
    description: 'Smash the targets as fast as you can!',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Easy',
    icon: '🔨',
  },
  {
    id: 'animal-balance',
    name: 'Animal Balance',
    description: 'Drop animals in a single line. If they fall, you lose!',
    category: 'ARCADE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-8 min',
    difficulty: 'Hard',
    icon: '🦒',
  },
  {
    id: 'ping-ball',
    name: 'Ping Ball',
    description: 'Keep the ball bouncing. Don\'t let it drop!',
    category: 'ARCADE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🏓',
  },
  {
    id: 'knife-thrower',
    name: 'Knife Thrower',
    description: 'Throw knives at the spinning log. Don\'t hit another knife!',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Hard',
    icon: '🗡️',
  },
  {
    id: 'fruit-ninja',
    name: 'Fruit Slash',
    description: 'Slice the flying fruits but avoid the bombs!',
    category: 'REACTION',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '🍉',
    featured: true,
  },
  {
    id: 'cornhole',
    name: 'Cornhole',
    description: 'Toss the beanbag into the hole for points!',
    category: 'ARCADE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '4-6 min',
    difficulty: 'Medium',
    icon: '🎯',
  },
  {
    id: 'coop-puzzle',
    name: 'Coop Puzzle',
    description: 'Work together to arrange the puzzle pieces and reveal the image!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-6 min',
    difficulty: 'Medium',
    icon: '🧩',
  },
  {
    id: 'water-sort',
    name: 'Water Sort',
    description: 'Sort the colored liquids into the correct tubes!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-10 min',
    difficulty: 'Hard',
    icon: '🧪',
  },
  {
    id: 'sudoku',
    name: 'Sudoku Co-op',
    description: 'Solve the Sudoku board together! Difficulty increases each round.',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-15 min',
    difficulty: 'Hard',
    icon: '🔢',
  },
  {
    id: '2048',
    name: '2048 Co-op',
    description: 'Combine matching tiles to reach 2048 together!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-10 min',
    difficulty: 'Medium',
    icon: '🧩',
  },
  {
    id: 'math-24',
    name: 'Math 24',
    description: 'Use the four numbers and basic math operations to reach exactly 24 before your opponents!',
    category: 'MIND',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-8 min',
    difficulty: 'Hard',
    icon: '➗',
  },
  {
    id: 'chain-reaction',
    name: 'Chain Reaction',
    description: 'Place orbs to explode into neighbor cells. Wipe out the opponent!',
    category: 'STRATEGY',
    minPlayers: 2,
    maxPlayers: 5,
    estimatedMinutes: '5-10 min',
    difficulty: 'Hard',
    icon: '💥',
  },
  {
    id: 'ludo',
    name: 'Ludo Master Duel',
    description: 'Animated 2-player Ludo board race! Roll 6 to spawn tokens, capture opponent tokens, and race home.',
    category: 'BOARD',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '5-10 min',
    difficulty: 'Medium',
    icon: '🎲',
    featured: true,
    isNew: true,
  },
  {
    id: 'escape-room',
    name: 'Cyber Escape Room',
    description: 'Co-op and competitive hacker room escape! Solve code locks, wire cutting, and terminal keypads.',
    category: 'ESCAPE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '4-8 min',
    difficulty: 'Hard',
    icon: '🔐',
    featured: true,
    isNew: true,
  },
  {
    id: 'riddle-duel',
    name: 'Emoji Riddle Showdown',
    description: 'Rapid-fire emoji riddle and trivia duel! Pick the right answer before your rival.',
    category: 'RIDDLES',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '3-5 min',
    difficulty: 'Medium',
    icon: '❓',
    featured: true,
    isNew: true,
  },
  {
    id: 'code-breaker',
    name: 'Mastermind Code Breaker',
    description: 'Deduce the secret 4-color code! Use black & white peg hints to crack the vault.',
    category: 'BRAIN',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '4-7 min',
    difficulty: 'Hard',
    icon: '🗝️',
    isNew: true,
  },
  {
    id: 'sliding-puzzle',
    name: 'Sliding Tile Race',
    description: 'Real-time 3x3 tile sliding speed race! Rearrange tiles in numerical order 1 to 8 first.',
    category: 'PUZZLE',
    minPlayers: 1,
    maxPlayers: 5,
    estimatedMinutes: '2-4 min',
    difficulty: 'Medium',
    icon: '🧩',
    isNew: true,
  }
];

export const FIND_MATCH_SYMBOLS = [
  'dice', 'ball', 'star', 'hammer', 'chest',
  'horseshoe', 'gem', 'hourglass', 'camera', 'drop',
  'lock', 'key', 'shield', 'feather', 'mushroom',
  'lightning', 'heart', 'crown', 'anchor', 'bell',
  'bomb', 'compass', 'crystal', 'flame', 'flower',
  'ghost', 'globe', 'leaf', 'moon', 'ring',
  'rocket', 'scroll', 'snowflake', 'sword', 'trophy',
  'apple', 'bone', 'boot', 'bottle', 'bow',
  'bug', 'candle', 'clover', 'crab', 'diamond',
  'egg', 'fan', 'fish', 'fist', 'flag',
  'guitar', 'hat', 'headphone', 'icecream', 'lantern',
  'lollipop', 'map', 'mask', 'medal', 'mirror',
] as const;

export type FindMatchSymbolId = typeof FIND_MATCH_SYMBOLS[number];

export const FIND_MATCH_SYMBOL_COUNTS: Record<string, number> = {
  easy: 5,
  normal: 7,
  hard: 9,
  insane: 11,
};

export const COLOR_CLASH_COLORS = ['red', 'blue', 'green', 'yellow', 'purple', 'orange'];
export const COLOR_CLASH_WORDS = ['RED', 'BLUE', 'GREEN', 'YELLOW', 'PURPLE', 'ORANGE'];
