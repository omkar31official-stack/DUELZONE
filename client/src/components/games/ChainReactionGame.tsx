...
export const ChainReactionGame: React.FC<{ socket: Socket; state: any; currentPlayer: Player | null; room: RoomSnapshot }> = ({ socket, state, currentPlayer, room }) => {
  ...
  return (
    <div className="w-full max-w-4xl flex flex-col items-center">
      <h2 className="text-4xl font-display text-white mb-6 drop-shadow-[0_0_10px_rgba(124,58,237,0.8)]">CHAIN REACTION</h2>
      ...
      {state.phase === 'playing' && (
        <div className="grid grid-cols-4 gap-2">
          {Array.from({ length: 16 }).map((_, idx) => (
            <button key={idx} onClick={() => { sounds.playClick(); socket.emit('game:action', { type: 'CLAIM', payload: { cellId: idx } }); }} className={`w-16 h-16 border-2 font-display text-2xl ${state.cells?.[idx] === currentPlayer?.id ? 'bg-brand-primary border-white' : 'bg-brand-card border-brand-secondary'}`}>
              {state.cells?.[idx] ? '💥' : ''}
            </button>
          ))}
        </div>
